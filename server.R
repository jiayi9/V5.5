...
server <- function(input, session,output) {
  ...
  getATTR = function(X,initial_max_N_level){
    X[] = lapply(X,as.character)
    X[is.na(X)|X==""]="N/A"
    LEVELS = sapply(X,function(x) length(unique(x)))
    R = X[,LEVELS>1&LEVELS<initial_max_N_level, drop=FALSE]
    return(R)
  }
  ...
  attr_names = reactive({
    ...
    TEMP = data_ranked_Chisq(X[,rejected_names],STATUS,floor(nrow(X)*0.05))$DATA
  ...
  attr_ranked_fit_2 = reactive({
    ...
    LEVELS = sapply(X,function(x) length(unique(x)))
    temp = sum(LEVELS<2)
    validate(need(temp==0,""))
    
    STATUS = STATUS()
    X[] = lapply(X,as.character)
    fit=data_ranked_Chisq(X,STATUS,floor(nrow(X)*0.05))
    fit
  })
...
#     PSN = upload()[,toupper(names(upload())) %in% toupper(c("Head.Phys.Psn","HEAD_PHYS_PSN"))]
    ...
    #X5[,!(toupper(names(upload())) %in% toupper(c("Head.Phys.Psn","HEAD_PHYS_PSN")))]
    
    LEVELS = sapply(X5,function(x) length(unique(x)))
    X6 = X5[,LEVELS>1&LEVELS<=MAXLEVEL(),drop=FALSE]
    
    X6
  })
  
  STATUS_drive = reactive({
    STATUS = STATUS()

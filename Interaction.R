...
output$BOX_ATTR <- renderUI({
  selectInput("box_attr", "Attribute:",  choices = names(attr_ranked_p_re()),selected = names(attr_ranked_p_re())[1])
}) 

output$BOX_PARA <- renderUI({
  selectInput("box_para", "Parametric:",  choices = names(para_ranked_fit()$DATA),selected = names(para_ranked_fit()$DATA)[1])
}) 
...
output$custom_boxplot = renderPlot({
  withProgress(message = 'Making custom boxplot', {
    if(is.null(input$box_attr) & is.null(input$box_para)){
      ...
      attr = attr_ranked_p_re()[,input$box_attr]
      para = para_ranked_fit()$DATA[,input$box_para]
      FAIL = STATUS()
      xname1 = input$box_attr
      xname2 = input$box_para
    }
    PlotLogistGroup_jitter(attr,para,STATUS(),xname1,xname2,0.4,1)
  })
})
